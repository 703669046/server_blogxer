...
class Upload extends BaseApi
{
    ...
    public function logo()
    {
        ...
        $info = $file->validate(['size' => 10*1024*1024, 'ext'=>'jpg,jpeg,png,gif,mp4'])->move(ROOT_PATH .'public' . DS . 'uploads' . DS . $type);
        if($info){
    ...
    public function images(){
        ...
        foreach ($files as $file){
            //移动文件到指定的目录下  /public/uploads/goods/目录下
            $dir=ROOT_PATH.'public'.DS.'uploads'.DS.$type;
            if(!is_dir($dir)){
                mk_dir($dir);
            }
            $info=$file->validate(['size'=>10*1024*1024,'ext'=>'jpg,jpeg,png,gif,mp4'])->move($dir);
            if($info){
                //成功 拼接图片路径
                $path=DS.'uploads'.DS.$type.DS.$info->getSaveName();
                $data['success'][]=$path;
            }else{
                //失败返回错误信息  getInfo()获取文件原始信息  getError（）获取错误信息
                $data['error'][]=[
