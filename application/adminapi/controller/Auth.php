...
class Auth extends BaseApi
{
    ...
    public function index()
    {
        ...
        //查询数据
        $list = \app\common\model\Auth::field('id,auth_name,pid,pid_path,auth_c,auth_a,is_nav,level,sort')->where($where)->order(['order','sort'=>'ASC'])->select();
        //转化为标准的二维数组
    ...
    public function nav()
    {
        ...
        if($role_id == 1){
            ...
            $data = \app\common\model\Auth::where($where)->order(['sort','sort'=>'desc'])->select();
        }else{
            //先查询角色表  role_auth_ids
            $role = \app\common\model\Role::find($role_id);
            $role_auth_ids = $role['role_auth_ids'];
            //再查询权限表
            $data = \app\common\model\Auth::where($where)->where('id', 'in', $role_auth_ids)->select();
        }
        foreach ($data as $item) {
            if($item['is_d']==1||$item['is_login']==1){
                $item['is_d']=true;
                $item['is_login']=true;
            }else{
                $item['is_d']=false;
